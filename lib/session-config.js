...
const ALWAYS_RESERVED_TEMPLATE_VARIABLES = new Set([
  '__proto__',
  'prototype',
  'constructor',
]);

function isReservedTemplateVariable(name, { allowExamples = false } = {}) {
  if (ALWAYS_RESERVED_TEMPLATE_VARIABLES.has(name)) return true;
  // When shared examples are enabled, {{examples}} is owned by that feature.
  return allowExamples && name === 'examples';
}
...
function normalizeTemplateFields(raw) {
  ...
  for (const item of raw) {
    ...
    if (
      ...
      || isReservedTemplateVariable(name)
      || seen.has(name)
    ) {
      continue;
    }
...
export function normalizeSessionConfig(raw) {
  ...
  const dynamicVariableNames = promptTemplating.dynamicFields
    ? [...new Set([
      ...
      name !== 'input' && !isReservedTemplateVariable(name, {
        allowExamples: promptTemplating.allowExamples,
      })
    ))
