...
export function renderPromptTemplate(template, input, options = {}) {
  ...
  if (options.strict && examples.length > 0 && !safeTemplate.includes(EXAMPLES_PLACEHOLDER)) {
    const err = new Error('Add {{examples}} to the prompt template to place the shared examples');
