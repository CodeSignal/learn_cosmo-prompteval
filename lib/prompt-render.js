...
export function renderPromptTemplate(template, input, options = {}) {
  ...
  // Shared structured examples win when provided. Otherwise keep a freeform
  // case/variable `examples` string (useful when allowExamples is off).
  if (examples.length > 0 || !Object.hasOwn(values, 'examples')) {
    values.examples = formatPromptExamples(examples);
  }
