 ...
 * @property {import('./llm/types.js').LlmProvider} [judgeLlm]
...
export async function runSingleEval(deps, opts) {
  ...
  try {
    ...
    const score = await scoreOutputAsync(
      output,
      opts.expectedAnswer,
      opts.metricId ?? DEFAULT_METRIC_ID,
      {
        llm: deps.judgeLlm ?? deps.llm,
        model: deps.judgeModel ?? model,
      },
    );
 ...
 *   onRunStart?: (info: { run: number }) => void,
 *   onRunDone?: (info: { run: number, result: EvalResult }) => void,
 * }} opts
 */
export async function runEvalBatch(deps, opts) {
  ...
  const renderedPrompt = renderPromptTemplate(promptTemplate, input, {
    ...
    strict: opts.strictTemplating === true,
  });
  const schedule = typeof opts.schedule === 'function'
    ? opts.schedule
    : createConcurrencyLimiter(opts.maxConcurrency).schedule;
  const onRunStart = typeof opts.onRunStart === 'function' ? opts.onRunStart : null;
  const onRunDone = typeof opts.onRunDone === 'function' ? opts.onRunDone : null;
  ...
  const results = await Promise.all(
    Array.from({ length: runs }, (_, i) =>
      schedule(async () => {
        ...
        onRunDone?.({ run, result });
        return result;
      }),
    ),
  );

  const scoringEnabled = isScoringEnabled(metricId, expectedAnswer);
