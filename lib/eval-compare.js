...
import { findPromptPlaceholders, renderPromptTemplate } from './prompt-render.js';
 ...
 *   promptTemplating?: { enabled?: boolean, allowExamples?: boolean, variableNames?: string[], dynamicFields?: boolean },
 * }} opts
 * @returns {Array<{
 *   id: string,
 *   label: string,
...
export function normalizeCases(opts = {}) {
  const templatingEnabled = opts.promptTemplating?.enabled === true;
  const variableNames = templatingEnabled
    ? (opts.promptTemplating?.dynamicFields
      ? [...new Set((opts.prompts ?? []).flatMap((prompt) => (
        findPromptPlaceholders(prompt?.promptTemplate)
      )))].filter((name) => {
        if (['input', '__proto__', 'prototype', 'constructor'].includes(name)) return false;
        if (name === 'examples' && opts.promptTemplating?.allowExamples === true) return false;
        return true;
      })
      : (Array.isArray(opts.promptTemplating?.variableNames)
        ? opts.promptTemplating.variableNames
        : []))
    : [];
 ...
 * @param {PromptVariant[]} prompts
 * @param {Array<{ input: string, variables?: Record<string, string> }>} cases
 * @param {Array<{ input?: string, idealOutput?: string }>} examples
 * @param {{ enabled?: boolean }} promptTemplating
 */
function assertRenderedPrompts(prompts, cases, examples, promptTemplating) {
  for (const testCase of cases) {
    for (const p of prompts) {
      const renderedPrompt = renderPromptTemplate(p.promptTemplate, testCase.input, {
        variables: testCase.variables,
        examples,
        strict: promptTemplating?.enabled === true,
      });
 ...
 *   promptTemplating?: { enabled?: boolean, variableNames?: string[] },
 *   metricId?: string,
 *   runs?: number,
...
export async function runPromptComparison(deps, opts) {
  ...
  const examples = opts.promptTemplating?.enabled === true
    && opts.promptTemplating?.allowExamples === true
  ...
  assertRenderedPrompts(prompts, cases, examples, opts.promptTemplating);
      ...
      Promise.all(
        prompts.map(async (p) => {
          ...
          const batch = await runBatch(deps, {
            ...
            templateVariables: testCase.variables,
            examples,
            strictTemplating: opts.promptTemplating?.enabled === true,
  ...
  const caseResults = cases.map((testCase, i) => {
    ...
    return {
      ...
      ...(opts.promptTemplating?.enabled ? { variables: testCase.variables } : {}),
  ...
  const result = {
    ...
    ...(opts.promptTemplating?.enabled
      ? { templateContext: { examples } }
      : {}),
    cases: caseResults,
